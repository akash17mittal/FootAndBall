...
def draw_bboxes(image, detections):
    ...
    for box, label, score in zip(
        ...
        if label == PLAYER_LABEL:
            ...
            cv2.putText(image, '{:0.2f}'.format(score), (int(
                x1), max(0, int(y1) - 10)), font, 1, color, 2)
            ...
            cv2.putText(image, '{:0.2f}'.format(score), (max(
                0, int(x - radius)), max(0, (y - radius - 10))), font, 1, color, 2)
